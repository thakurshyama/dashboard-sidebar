...
function GetApi() {
  const [isModalOpenpreparing, setIsModalOpenpreparing] = useState(false);
  const [isModalOpenpreparing2, setIsModalOpenpreparing2] = useState(false);
  const [isModalOpenpreparing3, setIsModalOpenpreparing3] = useState(false);
    ...
    const fetchUsers = async () => {
      try {
        ...
        const res = await axios.get(
          `http://139.59.60.119:4777/v1/order/outlets?status=preparing&from=2023-01-19&to=${d.toDateString()}`,
  ...
  const handleOpenModal = (user) => {
    setUserData(user);
    setIsModalOpenpreparing(true);
  };
  const handleOk = () => {
    setIsModalOpenpreparing(false);
  };
  ...
  const handleOpenModal2 = (user) => {
    setUserData(user);
    setIsModalOpenpreparing2(true);
  };
  const handleOk2 = () => {
    setIsModalOpenpreparing2(false);
  };
  ...
  const handleOpenModal3 = (user) => {
    setUserData(user);
    setIsModalOpenpreparing3(true);
  };
  const handleOk3 = () => {
    setIsModalOpenpreparing3(false);
  };
  ...
  return (
    <div className='New'>
      ...
      <Modal title='User Information' open={isModalOpenpreparing} onOk={handleOk} onCancel={handleCloseModal}>
        {userData && (
          <div>
            <p>
      ...
      <Modal title='User Information' open={isModalOpenpreparing2} onOk={handleOk2} onCancel={handleCloseModal2}>
        {userData && (
          <div>
            <p>
      ...
      <Modal title='User Information' open={isModalOpenpreparing3} onOk={handleOk3} onCancel={handleCloseModal3}>
        {userData && (
          <div>
            <p>
